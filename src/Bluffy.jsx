...
import QUESTIONS_EN from "./EnglishQuestions.json";
import FLAGS from "./Flags_hebrew_and_english.json";
function buildQuestionPool() {

  // Normal questions
  const normal = QUESTIONS_EN.map(q => ({
    id: String(q.id),
    category: q.category.toLowerCase().replace(/\s+/g, "_"),
...
export default function Bluffy(){
  ...
  const ALL_QUESTIONS = buildQuestionPool();
  ...
  const categoryPool = ALL_QUESTIONS.filter(
    q => q.category === cat && !usedIds.includes(q.id)
  );
  ...
  await update(ref(db, `rooms/${room}`), {
    ...
    question: q,
    cat,
    deadline: Date.now() + (rd?.settings?.time || 30) * 1000,
  ...
  const submitAnswer=()=>{
   ...
   const ok = isCorrect(ci, q.answer_en);
  ...
  const submitBluff=()=>{
    ...
    const q=rd.question;const ln=rd.lang||lang;
    if(strictMatch(bi,q.answer_en)||strictMatch(bi,q.answer_en)||isCorrectBilingual(bi,q.answer_en,q.answer_en))return;
  ...
  const skipBluff=()=>{
    ...
    const q=rd.question;const ln=rd.lang||lang;const ex=[...Object.values(rd.publicAnswers||{}),q.answer_en,q.answer_en];
  ...
  const hostAutoProgress=()=>{
    ...
    playerList.forEach(([id])=>{
      ...
      if(!rd?.publicAnswers?.[id]){
        ...
        if(a?.ok){const q=rd.question;u[`publicAnswers/${id}`]=genDecoy(q,[...Object.values(rd.publicAnswers||{}),q.answer_en,q.answer_en],ln);}
  ...
  const hostMoveToReveal=()=>{
    ...
    const q=rd.question;const ln=rd.lang||lang;
    const ca=ln==="he"?q.answer_en:q.answer_en;
    ...
    Object.entries(rd.publicAnswers||{}).forEach(([id,txt])=>{
      ...
      if(isCorrectBilingual(txt,q.answer_en,q.answer_en))return;
  ...
  if(state==="answering"){const q=rd.question;return(<div style={{minHeight:"100vh",background:bg,padding:20,direction:he?"rtl":"ltr"}}><div style={{maxWidth:500,margin:"0 auto"}}>
    ...
    <div style={{...C,marginBottom:16,textAlign:"center",borderColor:"rgba(255,215,0,.2)"}}>
      ...
      {rd.question?.type === "flag" && (
  <img
    src={rd.question.flag}
    alt=""
    style={{
      width: 160,
      height: 100,
